...
const loadBannedWords = threadID => {
  const wordFile = path.join(__dirname, `../../mods/cmds/${threadID}.json`);
...
module.exports.run = async function({ api, event, args, message }) {
  ...
  switch (action) {
    case 'add':
      ...
      fs.writeFileSync(path.join(__dirname, `../../mods/${threadID}.json`), JSON.stringify(bannedWords[threadID]), "utf8");
    ...
    case 'remove':
      ...
      if (index !== -1) {
        ...
        fs.writeFileSync(path.join(__dirname, `../../mods/cmds${threadID}.json`), JSON.stringify(bannedWords[threadID]), "utf8");
